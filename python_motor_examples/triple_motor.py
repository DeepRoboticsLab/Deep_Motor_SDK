...
class Can:
    def __init__(self, interface_name="can0", bitrate=1000000):
        ...
        #将三个关节id分别设为0,1,2,若与实际的关节id不符，请在代码中将self.idX修改为实际的关节id
        self.id0 = 0
        self.id1 = 1
        self.id2 = 2
        self.can0 = None
    ...
    def can_func(self):
        ...
        while True:
            print(i)
            i += 1
            self.motor(self.id0, i)
            self.motor(self.id1, i)
            self.motor(self.id2, i)

    def motor(self, id, i):
        if i == 1:
            self.can_send(id=id, cmd=1)
        elif i == 2:
            self.can_send(id=id, cmd=2)
        else:
            self.can_send(id=id, cmd=4)
        self.can_recv(id=id, cmd=4)

    def can_send(self, id, cmd):
        try:
            data_send_can = FormSendData(cmd=cmd, id=id, data_send_orgin=[0, 0, 0, 0, 1])
    ...
    def can_recv(self, id, cmd):
        try:
            ...
            can_id = (cmd << 5) | (id + 0x10)
            data_recv_can = ParseRecvData(id=can_id, data_recv_orgin=data)
            logging.info(f"{id}Received CAN message: {data_recv_can}")
        except Exception as e:
